    ...
    <div class="container">
      <div class="bread-crumb">
        <Breadcrumb>
          ...
          <Tag
            v-for="(value, name, index) in search.filter"
            :key="index"
            closable
            @on-close="closeTags(name)"
      ...
      <div class="item-class-show">
        ...
        >
          ...
          <i-col class="item-class-select" span="21" v-if="f.k !== 'cid3'">
            <span
              v-for="(o, j) in f.options"
              :key="j"
              @click="selectFilter(f.k, o)"
              >{{ o.name }}</span
            >
          </i-col>
        ...
        <Row class="item-class-group" v-if="filters.length > 5">
          ...
          <i-col class="item-class-select" span="21">
            <span
              v-bind:style="foldFilter.k == f.k ? 'color:red' : 'color:black'"
              v-for="(f, i) in filters"
              :key="i"
              v-if="i > 5"
              @click="showFold(f)"
              >{{ f.k }} <Icon type="ios-arrow-up" v-if="foldFilter.k != f.k"/>
              <Icon type="ios-arrow-down" v-if="foldFilter.k == f.k"
            /></span>
          </i-col>
        </Row>
        <Row>
          <i-col class="item-class-select" span="24">
            <span
              v-for="(o, j) in foldFilter.options"
              :key="j"
              @click="selectFilter(foldFilter.k, o)"
              >{{ o.name }}</span
            >
          </i-col>
        </Row>
            ...
            >
              <div class="goods-show-img">
                <router-link to="/goodsDetail"
                  ><img :src="item.selected.image" height="200"
                /></router-link>

                <ul class="skus">
                  <li
                    :class="{ selected: sku.id === item.selected.id }"
