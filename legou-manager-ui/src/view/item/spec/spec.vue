  ...
  <div>
    ...
    <Modal v-model="modal" width="800px">
      ...
      <Card v-for="(groupItem, groupIndex) in groups" :key="groupIndex">
        ...
        <Collapse>
          <Panel v-for="(paramItem, paramIndex) in groupItem.params" :name="`${groupIndex}-${paramIndex}`"
                 :key="paramIndex">
            {{ paramItem.name }}
            <Icon type="ios-trash" style="float: right;margin-top: 10px"
                  @click="deleteParam(groupItem.id, paramItem.id)"></Icon>
            <div slot="content" style="height: 40px;">
              ...
              <Col span="4" style="margin-top: 5px; margin-left: 20px;">
                数字参数：
                <i-switch v-model="paramItem.numeric"/>
              </Col>
              <Col span="4" style="margin-top: 5px; margin-left: 20px;">
                搜索参数：
                <i-switch v-model="paramItem.searching"/>
              </Col>
              <Col span="4" style="margin-top: 5px; margin-left: 20px;">
                通用规格:
                <i-switch v-model="paramItem.generic"/>
              </Col>
            </div>
          </Panel>
        </Collapse>
