  ...
  <div>
    ...
    <div v-show="step == 3">
      <Card v-for="(svalue, skey, sindex) in specialSpec" :key="sindex">
        ...
        <Input v-for="(vvalue, vindex) in svalue" :key="vindex" v-model="specialSpec[skey][vindex]"
    ...
    <div v-show="step == 4">
      ...
      <Row style="border-bottom: solid #c3c3c3 1px; height: 80px; line-height: 80px" v-for="(sku, index) in spu.skus"
           :key="index">
  ...
  watch: {
    ...
    specialSpec: {
      handler (newValue, oldValue) {
        ...
        if (this.spu.spuDetail.specialSpec ===
